...
- **Evolution 管线**：`npm run evolution:learn` 拉 RSS → `doc/evolution/inbox/` + `skills/agent-tech-digest`；`npm run evolution:run-day` 对每条 inbox 链接先抓取正文摘录，再在本仓库 worktree 跑 `EVOLUTION_TEST_CMD`（默认不克隆外链仓库），默认最多 3 路并行（`EVOLUTION_CONCURRENCY`，`AUTO_MERGE=1` 时串行），写 `doc/evolution/success|failure` 与 `doc/evolution/runs/latest-run-day.md`；`EVOLUTION_AUTO_MERGE=0` 默认不合并；定时见 `scripts/cron-evolution.example.sh`
