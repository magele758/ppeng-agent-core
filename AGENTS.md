...
- **`@ppeng/agent-loop`（L4 可嵌入 Agent Loop SDK）**：代码在 `packages/agent-loop/`（公开发布名 `@mage-ai-lab/agent-loop`）；提供 mini / normal / full / max 四个装配档位（`mini` 严格禁止静态引入任何 Node 内置模块，保证无 Node I/O 便携性；重模块动态按需加载）；提供 `createAssembledLoop` / `createAgentLoop`（L4 `step()` / `run()` / `for await` / `steer()` / `abort()` / `fold()`）与 L3 `runSessionKernel`；单测命令 `cd packages/agent-loop && npm run test`。**更新该 SDK 时必须同步更新 `skills/agent-loop/SKILL.md`**（见「Learned User Preferences」核心指令）。
