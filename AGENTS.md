...
- **Evolution 管线**：`npm run evolution:learn` 拉 RSS → `doc/evolution/inbox/` + `skills/agent-tech-digest`；`npm run evolution:run-day` 按 inbox 建 worktree、跑 `EVOLUTION_TEST_CMD`（默认 `npm run test:unit`）、写 `doc/evolution/success|failure`；`EVOLUTION_AUTO_MERGE=0` 默认不合并；定时见 `scripts/cron-evolution.example.sh`
