...
- **并行**：`EVOLUTION_CONCURRENCY` 控制同时跑几条（默认与上限见 `EVOLUTION_CONCURRENCY_MAX`，默认 **5**，硬顶 **64**）。`EVOLUTION_AUTO_MERGE=1` 时各 worktree 内构建/测试仍可并行；**合并进主分支**在脚本内 **串行（互斥）**，避免多路同时操作主仓。
