...
export async function maybeExportOtelSpan(
  ...
  const traceId = Buffer.from(
    `${sessionId}:${name}:${Date.now()}`.slice(0, 32).padEnd(32, '0')
  )
    .toString('hex')
    .slice(0, 32);
  const spanId = Buffer.from(`${Date.now()}`).toString('hex').slice(0, 16);
  const body = {
    resourceSpans: [
      {
        resource: { attributes: [{ key: 'service.name', value: { stringValue: 'raw-agent' } }] },
        scopeSpans: [
          {
            spans: [
              {
