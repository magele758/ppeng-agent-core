...
export class RawAgentRuntime {
  ...
  private async expandHttpMcpTools(
    ...
    if (urls.length === 0 || this.tools.some((t) => t.name === 'mcp_invoke')) return;
    ...
    for (let hi = 0; hi < httpUrls.length; hi++) {
      ...
      for (const t of listed) {
        ...
        if (this.tools.some((x) => x.name === name)) continue;
  ...
  private registerResourceTools(
    ...
    if (!this.tools.some((t) => t.name === 'mcp_list_resources')) {
      this.tools.push({
    ...
    if (!this.tools.some((t) => t.name === 'mcp_read_resource')) {
      this.tools.push({
