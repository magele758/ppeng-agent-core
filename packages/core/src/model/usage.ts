...
/**
 * Some gateways report `prompt_tokens` as a running total for the whole session
 * rather than for the request that just finished. Summing those verbatim makes
 * session totals — and therefore the USD estimate — grow quadratically: a 30k
 * context over 10 turns is reported as ~433k input tokens.
 *
 * Detection is two-stage, because a purely relative gate is not enough. The
 * first cumulative report is recognized by a large jump past the previous figure
 * (≥ +40% and at least +1000 tokens). After that the verdict is **sticky**: a
 * provider does not switch reporting style mid-session, and as a running total
 * grows the per-turn increment becomes a smaller and smaller fraction of it, so
 * a relative gate alone would silently start reading late turns as per-request
 * and re-inflate the totals.
 *
 * A context that merely grew turn over turn stays below the first-detection
 * gate, so a provider with honest per-request usage is never misread.
 *
 * @param incoming prompt/input tokens reported by this turn
 * @param previousCumulative highest figure seen so far this session
 * @param alreadyCumulative true once this session has been classified cumulative
 */
export function splitCumulativePromptTokens(
  incoming: number,
  previousCumulative: number | undefined,
  alreadyCumulative = false
): {
  turnInputTokens: number;
  cumulativeInputTokens: number;
  treatedAsCumulative: boolean;
} {
  const prev = previousCumulative ?? 0;
  const bigJump =
    prev > 0 && incoming > prev && incoming >= prev + Math.max(1000, prev * 0.4);
  // Sticky, but only while the figure keeps climbing: a drop means the provider
  // is reporting this request (e.g. the prompt shrank after compaction).
