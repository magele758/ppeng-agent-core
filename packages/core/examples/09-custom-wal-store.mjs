 ...
 * `createMemorySurfaceStore` is a Phase 2 sibling; until it lands this uses
 * the exported `SqliteStateStore` in a temp dir (still in-process, no HTTP).
 *
...
import { foldSurface } from '../dist/index.js';
...
const stateDir = mkdtempSync(join(tmpdir(), 'ppeng-l1-fold-'));
const store = new SqliteStateStore(join(stateDir, 'runtime.sqlite'));
...
const foldTexts = folded
  ...
  .join('|');
const walHasV1 = wal.some((m) => m.parts.some((p) => p.type === 'text' && p.text === 'note-v1'));
...
console.log('WAL rows:', wal.length, 'fold rows:', folded.length);
console.log('fold texts:', foldTexts);

if (wal.length < 4) fail('WAL should keep originals including hidden/replaced keys');
if (!walHasV1) fail('WAL listMessages should still contain note-v1');
